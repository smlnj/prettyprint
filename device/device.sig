...
sig
  ...
  type token (* "physical", or "device", token *)
    ...
    *     via a tokenmap function that is user defined based on a concrete "device" structure
    *     that provides additional interface elements (say lambda: token).
    * (3) The "physical" size of a device token, i.e. the number of spaces it takes up when
    *     output, should agree with the size attribute of the logical token that it is derived
    *     from via a tokenmap parameter supplied to the render function.
    * (4) Typically, the device token type will be string, as in PPDevice/src/ansi-term-dev.sml.
    *)
