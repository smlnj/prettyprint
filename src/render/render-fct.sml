(* prettyprint/src/render-fct.sml *)
...
(* --------------------------------------------------------------------------------
 *  RenderFn: the Rendering functor, taking a device D as argument
 *    D will be bound to plain and and ansiterm devices: PlainDevice, ANSITermDevice, respectively,
 *    to create PlainRender and ANSITermRender rendering structures.
...
fun render (format: F.format, lineWidth: int) : unit =
	...
	fun flatRender format =
		...
		fun render0  (format: F.format): unit =
			...
			of F.EMPTY => ()
			 | F.TEXT s => D.string s
			 | F.TOKEN t => D.string (Token.raw t)
	...
	fun render1  (format: F.format, cc: int, newlinep: bool) : int * bool =
		  ...
		  | F.TOKEN t =>  (* print the string unconditionally; move cc accordingly *)
		      (D.string (T.raw t); (cc + T.size t, false))
