(* prettyprint/src/ansiterm-device-fct.sml *)

(* The structure ANSITermDevice itself represents an ANSI terminal as output device.
 * There is no device "type" in this representation -- just a particular device represented
 * as a structure produced by the ANSITermDevice functor. This is necessary to provide
 * a polymorphic renderStyle function. *)

functor ANSITermDeviceFn (Out: sig val outstream: TextIO.outstream end): DEVICE =
struct

local

  structure T = ANSITerm      (* SMLNJ-LIB/Util: provides type ANSITermStyle *)
  structure S = ANSITermStyle (* ansiterm-style.sml *)
...
(* output a string/character in the current style to the device *)
fun token (t: Token.token) = string (Token.raw t)
...
fun setTermMode cmd =
    TextIO.output (Out.outstream, T.toString [cmd])
...
fun applyStyle (style: S.ansiTermStyle)  : (unit -> unit) option =
    case style
      of S.FG new_fg =>
	   if not (!fg = new_fg)
...
(* renderStyled : Style.style * (unit -> 'a) -> 'a *)
fun 'a renderStyled (style: Style.style, renderFormat : unit -> 'a) : 'a =
    let val post = applyStyle (S.styleToAnsiTermStyle style)
