...
local
  ...
  structure T = ANSITerm (* provides type ANSITermStyle *)
  structure S = Style
...
fun applyStyle (style: T.ANSITermStyle)  : (unit -> unit) option =
    case style
      of S.FG new_fg =>
	   if not (!fg = new_fg)
...
fun 'a renderStyled (style: S.style, renderFormat : unit -> 'a) : 'a =
    let val post = applyStyle (T.styleToANSITermStyle style)
